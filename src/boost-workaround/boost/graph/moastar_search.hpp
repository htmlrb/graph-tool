...
#define BOOST_GRAPH_MOASTAR_SEARCH_HPP
...
/**
 * HEURISTIC AND VISITOR CONCEPTS
 * ****************************************
 */
template<class Heuristic, class Graph>
struct MOAStarHeuristicConcept {
	void constraints() {
...
class moastar_heuristic: public std::unary_function<
...
moastar_visitor<Visitors> make_moastar_visitor(Visitors vis) {
	return moastar_visitor<Visitors>(vis);
}
typedef moastar_visitor<> default_moastar_visitor;
...
struct mo_astar_bfs_visitor {
	...
	mo_astar_bfs_visitor(MOAStarHeuristic h, UniformCostVisitor vis,
			...
			m_h(h), m_vis(vis), m_Q(Q), m_predecessor(p), m_cost(c), m_distance(
					d), m_weight(w), m_color(col), m_combine(combine), m_compare(
					compare), m_zero(zero) {
	}
...
template<typename VertexListGraph, typename MOAStarHeuristic,
		typename MOAStarVisitor, typename PredecessorMap, typename CostMap,
		typename DistanceMap, typename WeightMap, typename ColorMap,
		typename VertexIndexMap, typename CompareFunction,
		typename CombineFunction, typename CostInf, typename CostZero>
inline void moastar_search_no_init(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, MOAStarVisitor vis, PredecessorMap predecessor,
...
template<typename VertexListGraph, typename MOAStarHeuristic,
		...
		typename CombineFunction, typename CostInf, typename CostZero>
inline void moastar_search_no_init_tree(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, MOAStarVisitor vis, PredecessorMap predecessor,
...
inline void moastar_search(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, MOAStarVisitor vis, PredecessorMap predecessor,
		CostMap cost, DistanceMap distance, WeightMap weight,
	...
	moastar_search_no_init(g, s, h, vis, predecessor, cost, distance, weight,
...
template<typename VertexListGraph, typename MOAStarHeuristic,
		...
		typename DistanceMap, typename WeightMap, typename CompareFunction,
		typename CombineFunction, typename CostInf, typename CostZero>
inline void moastar_search_tree(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, MOAStarVisitor vis, PredecessorMap predecessor,
		CostMap cost, DistanceMap distance, WeightMap weight,
		CompareFunction compare, CombineFunction combine, CostInf inf,
	...
	moastar_search_no_init_tree(g, s, h, vis, predecessor, cost, distance,
			weight, compare, combine, inf, zero);

}

// Named parameter interfaces
template<typename VertexListGraph, typename MOAStarHeuristic, typename P,
		typename T, typename R>
void moastar_search(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, const bgl_named_params<P, T, R>& params) {
	...
	moastar_search(g, s, h,
			arg_pack[_visitor | make_moastar_visitor(null_visitor())],
			arg_pack[_predecessor_map | dummy_property_map()],
			detail::make_property_map_from_arg_pack_gen<tag::rank_map, D>(D())(
					g, arg_pack),
			detail::make_property_map_from_arg_pack_gen<tag::distance_map, W>(
					W())(g, arg_pack),
			detail::override_const_property(arg_pack, _weight_map, g,
					edge_weight),
			...
			arg_pack[_distance_combine | closed_plus<D>(inf)], inf,
...
template<typename VertexListGraph, typename MOAStarHeuristic, typename P,
		typename T, typename R>
void moastar_search_tree(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, const bgl_named_params<P, T, R>& params) {
	using namespace boost::graph::keywords;
	typedef bgl_named_params<P, T, R> params_type;
	...
	moastar_search_tree(g, s, h,
			...
			detail::make_property_map_from_arg_pack_gen<tag::distance_map, W>(
					W())(g, arg_pack),
			detail::override_const_property(arg_pack, _weight_map, g,
					edge_weight), arg_pack[_distance_compare | std::less<D>()],
			arg_pack[_distance_combine | closed_plus<D>(inf)], inf,
			arg_pack[_distance_zero | D()]);
}
...
template<typename VertexListGraph, typename MOAStarHeuristic, typename P,
		typename T, typename R>
void moastar_search_no_init(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, const bgl_named_params<P, T, R>& params) {
	using namespace boost::graph::keywords;
	typedef bgl_named_params<P, T, R> params_type;
	...
	moastar_search_no_init(g, s, h,
			...
			detail::make_property_map_from_arg_pack_gen<tag::rank_map, D>(D())(
					g, arg_pack),
			detail::make_property_map_from_arg_pack_gen<tag::distance_map, D>(
					D())(g, arg_pack),
			detail::override_const_property(arg_pack, _weight_map, g,
					edge_weight),
			detail::make_color_map_from_arg_pack(g, arg_pack),
			detail::override_const_property(arg_pack, _vertex_index_map, g,
					vertex_index), arg_pack[_distance_compare | std::less<D>()],
			arg_pack[_distance_combine | closed_plus<D>(inf)], inf,
			arg_pack[_distance_zero | D()]);
}
...
template<typename VertexListGraph, typename MOAStarHeuristic, typename P,
		typename T, typename R>
void moastar_search_no_init_tree(const VertexListGraph &g,
		typename graph_traits<VertexListGraph>::vertex_descriptor s,
		MOAStarHeuristic h, const bgl_named_params<P, T, R>& params) {
	using namespace boost::graph::keywords;
	typedef bgl_named_params<P, T, R> params_type;
...
#endif // BOOST_GRAPH_MOASTAR_SEARCH_HPP
